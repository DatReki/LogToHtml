...
namespace LogToHtml
{
	public class Logging
	{
		...
		/// <param name="error">A string of the error/exception you want to log</param>
		/// <returns>
		...
		///     </list>
		/// </returns>
		public static (DateTime date, LogType logType, string error) Log(Options options, LogType logType, string error)
		{
			...
			else
			{
				if (options.Projects.Contains(options.Project))
				{
					...
					if (options.LogToConsole)
					{
						...
						Console.WriteLine($"[{options.Project}] [{options.Date}] [{consoleError}] {error}");
					}
					...
					switch (fileExists)
					{
						case false:
							//Create New file
							WriteLog.CreateLog(options, logType, error);
							break;
						default:
							WriteLog.EditLog(options, logType, error);
							//Edit existing file
							break;
					}
				}
			...
			return (options.Date, logType, error);
		}
		...
		///     </list>
		/// </returns>
		public static (List<AllLogs.LogVariables> Critical, List<AllLogs.LogVariables> Error, List<AllLogs.LogVariables> Warning, List<AllLogs.LogVariables> Info) GetLogs()
		{
